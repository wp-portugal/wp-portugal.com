...
class Setting extends \DeliciousBrains\WPMDB\Common\Cli\Cli
{
	...
	protected function _has_valid_admin()
	{
		...
		$user   = $config['user'];
		if (!$user) {
			\WP_CLI::error(__('License requires specifying a user via --user=<id|login|email>', 'wp-migrate-db-pro-cli') );
		}
		if  (!current_user_can('administrator')) {
			\WP_CLI::error(__('User must be an Admin', 'wp-migrate-db-pro-cli') );
		}
