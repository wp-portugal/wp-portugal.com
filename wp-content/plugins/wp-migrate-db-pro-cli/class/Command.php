...
class Command extends \DeliciousBrains\WPMDB\Pro\Cli\Command
{
    ...
    {
        \WP_CLI::add_command('wpmdb', 'DeliciousBrains\WPMDBCli\WPMDBCLI_Deprecated'); // deprecated older command
        \WP_CLI::add_command('migratedb', 'DeliciousBrains\WPMDBCli\Command');
    }
    ...
    public function profiles()
    {
        ...
        foreach ($profiles as $key => $profile) {
            ...
            $action_strings = [
                ...
                'import'       => _x('import', 'Import data from SQL file', 'wp-migrate-db-pro-cli'),
                'backup_local' => _x('backup', 'Backup the local database', 'wp-migrate-db-pro-cli'),
            ];
