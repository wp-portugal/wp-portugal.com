...
class Cli extends Export
{
    ...
    public function get_profile_by_name($name)
    {
        ...
        return $this->cli_error(__('Profile not found.', 'wp-migrate-db-pro-cli'));
    }
    ...
    public function maybe_show_outdated_profile_error($profile)
    {
        ...
        if ($outdated) {
            ...
            $message = __('This profile is from an older version of WP Migrate DB Pro and some settings have changed.', 'wp-migrate-db');
            $message .= "<br />";
            $message .= sprintf(
    ...
    public function check_local_wpmdbpro_mst_before_migration($profile)
    {
        ...
        if (false === class_exists('\DeliciousBrains\WPMDBMST\MultisiteToolsAddon')) {
            return $this->cli_error(__('The profile is set to migrate between a single site and a multisite, however WP Migrate DB Pro Multisite Tools does not seem to be installed/active on the local website.', 'wp-migrate-db-pro-cli'));
        }
    ...
    public function check_remote_wpmdbpro_mst_before_migration($profile)
    {
        ...
        // We don't support transferring a subsite between multisites yet.
        if ($local_is_multisite && $remote_is_multisite) {
            if ($is_subsite_migration) {
                ...
                // We do allow regular MS -> MS migrations.
                return $profile;
            }
        }
    ...
    public function check_remote_wpmdbpro_tpf_before_migration($profile)
    {
        ...
        if ($tpf_enabled) {
            if (!isset($this->remote['theme_plugin_files_available'])) {
                return $this->cli_error(__('The profile is set to migrate theme/plugin files, however WP Migrate DB Pro Theme & Plugin Files does not seem to be installed/active on the remote website.', 'wp-migrate-db-pro-cli'));
            }
