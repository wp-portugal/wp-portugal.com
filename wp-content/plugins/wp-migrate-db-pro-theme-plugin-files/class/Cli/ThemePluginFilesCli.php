...
use DeliciousBrains\WPMDBTP\ThemePluginFilesFinalize;
...
class ThemePluginFilesCli extends ThemePluginFilesAddon
{
    ...
    public function register()
    {
        ...
        add_filter('wpmdb_cli_profile_before_migration', [$this, 'add_tpf_stages']);
    ...
    public function add_tpf_profile_args($profile, $args, $assoc_args)
    {
        ...
        if (isset($assoc_args['exclude-theme-plugin-files'])) {
            $theme_plugin_files['excludes'] = str_replace(',', "\n", $assoc_args['exclude-theme-plugin-files']);
        }
    ...
    public function get_themes_to_migrate($profile, $themes)
    {
        ...
        foreach ($themes_selected as $theme) {
            ...
            if (!isset($themes[$theme])) {
                $message = sprintf(__('Theme not found on source server: %s', 'wp-migrate-db-pro-theme-plugin-files'), $theme);
    ...
    public function get_plugins_to_migrate($profile, $plugins)
    {
        ...
        foreach ($plugins_selected as $plugin) {
            ...
            if (!isset($plugin_slugs[$plugin])) {
                $message = sprintf(__('Plugin not found on source server: %s', 'wp-migrate-db-pro-theme-plugin-files'), $plugin);
    ...
    public function initialize_tpf_migration($profile, $post_data, $stage)
    {
        ...
        if ('themes' === $stage) {
            $initiate_msg           = __('Initiating themes migration...', 'wp-migrate-db-pro-theme-plugin-files');
            ...
            $initiate_msg            = __('Initiating plugins migration...', 'wp-migrate-db-pro-theme-plugin-files');
        ...
        if (isset($profile['theme_plugin_files']['excludes'])) {
            $_POST['excludes'] = json_encode($profile['theme_plugin_files']['excludes']);
        }
