...
class WCB_Widget_Sponsors extends WP_Widget {
	...
	function WCB_Widget_Sponsors() {
		$widget_ops = array(
			...
			'description' => __( 'Your WordCamp&#8217;s Sponsors', 'wcb' ),
		);
		$this->WP_Widget( 'wcb_sponsors', __('Sponsors', 'wcb'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		foreach ( $terms as $term ):
			...
			while ( wcb_have_sponsors() ):
				wcb_the_sponsor();
				?><a class="sponsor-logo" href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'twentyten' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php
