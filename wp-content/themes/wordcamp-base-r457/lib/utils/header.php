...
function __wcb_post_single_meta( $meta ) {
	...
	if ( is_object_in_taxonomy( get_post_type(), 'category' ) ) {
		$meta['category'] = sprintf( __('Posted in %s', 'wcb'), get_the_category_list( ', ' ) );
	}
	if ( is_object_in_taxonomy( get_post_type(), 'tag' ) ) {
		$meta['tag'] = sprintf( __('Tagged %s', 'wcb'), get_the_tag_list( '', ', ' ) );
	}
...
function wcb_title_tag() {
	...
	if ( $paged >= 2 || $page >= 2 )
		echo ' | ' . sprintf( __( 'Page %s', 'wcb' ), max( $paged, $page ) );
	echo "</title>";
}
...
function wcb_typekit_scripts() {
	$kit_id = apply_filters( 'wcb_typekit_id', 'jnd4dds' );
